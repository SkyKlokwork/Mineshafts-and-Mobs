...
public class SpellDisplay {
    ...
    private final SpellFrame frame;
    ...
    public SpellDisplay(ItemStack icon, Text title, Text description, @Nullable Identifier background, SpellFrame frame, boolean showToast, boolean announceToChat, boolean hidden) {
        this.title = title;
    ...
    public static SpellDisplay fromJson(JsonObject obj) {
        ...
        ItemStack itemStack = SpellDisplay.iconFromJson(JsonHelper.getObject(obj, "icon"));
        Identifier identifier = obj.has("background") ? new Identifier(JsonHelper.getString(obj, "background")) : null;
        SpellFrame spellFrame = obj.has("frame") ? SpellFrame.forName(JsonHelper.getString(obj, "frame")) : SpellFrame.TASK;
        ...
        return new SpellDisplay(itemStack, text, text2, identifier, spellFrame, bl, bl2, bl3);
    }
    ...
    public static SpellDisplay fromPacket(PacketByteBuf buf) {
        Text text = buf.readText();
        Text text2 = buf.readText();
        ItemStack itemStack = buf.readItemStack();
        SpellFrame spellFrame = buf.readEnumConstant(SpellFrame.class);
        int i = buf.readInt();
        Identifier identifier = (i & 1) != 0 ? buf.readIdentifier() : null;
        boolean bl = (i & 2) != 0;
        boolean bl2 = (i & 4) != 0;
        SpellDisplay spellDisplay = new SpellDisplay(itemStack, text, text2, identifier, spellFrame, bl, false, bl2);
        spellDisplay.setPos(buf.readFloat(), buf.readFloat());
        return spellDisplay;
    }
